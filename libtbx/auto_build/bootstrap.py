...
if sys.version_info[0] > 2:
    print("Please ensure the first python in your path is python version 2, not 3")
    sys.exit()
import ntpath
import os
import os.path
...
{conda_env}""".format(conda_env=self.use_conda))
  ...
  def add_dispatchers(self, product_name="phenix"):
    ...
    envcmd = "export"
    dispatcher = os.path.join("build2",
...
that maintain their own conda environment.""",
  ...
  parser.add_argument("--build-dir", dest="build_dir", 
    help="directory where the build will be. Should be at the same level as modules!default it 'build'", 
    default="build", type=str)

  options = parser.parse_args()
  args = options.action

  # set the build folder to allow for multiple configuration builds side by side (e.g. cudabuild)
  global _BUILD_DIR 
  _BUILD_DIR = options.build_dir  # TODO: this is probably ok way to go, but check and see
